...
/* Allocate and fill array for the trapezoidal weights for line of sight integration */
  ...
  //herehere
  double fundamental_base = 1.34;  //prev on 2.52
  ...
  if (k_max < ppt->k[ppt->index_md_scalars][ppt->k_size[ppt->index_md_scalars]-1]) {
    printf("ERROR! SONG doesn't comput k_max as high as galbispectra2 demands. Please increase k_max_for_pk or reduce k_max in galbispectra2 \n" );
    exit(0);
  }
...
-------------------------------------------------*/
  ...
  /* Next we cut-off the effect window function by integrating from the tau value corresponding to the mean redshift to a lower time value,
  when the value of the integration exceeds some percentage (e.g. 49%) we take the lower_cutoff index and do the same for the higher time values to
  ...
  int half_size = (pgb2->alpha_log_size-1/2);
  double alpha_log_min = 0.488854; // prev on   0.7766
  double alpha_log_max = 1.08194; // prev on   0.7942

  double alpha_log_base = 1.34; //prev 1.02
  alpha_log_base = pow(alpha_log_base,1/pow(2,res_factor));



  for (int index_log_alpha = half_size; index_log_alpha > -1; index_log_alpha--) {
  ...
  for (int i = 0; i < ppt->selection_num; i++) {
    ...
    for (int j = 0; j < ppt->selection_num; j++) {
      ...
      for (int k = 0; k < pgb2->alpha_window_size; k++) {
        ...
        class_alloc(pp[i][j][k],
                    pgb2->r_size * sizeof(double),
                    pgb2->error_message);
      }
    }
  }
  printf("**pp-array allocated.\n" );
  ...
  /* Time debug PRINT SUN RAY*/
/*  double tau100, tau200, tau101, tau201, tau110, tau210, tau111, tau211;
  for (int index_alpha = 0; index_alpha < pgb2->alpha_size-1; index_alpha++) {
    //printf("alpha2[%d][%d][%d] = %g\n", 0, 0, index_alpha, alpha2[0][0][index_alpha]);
    //printf("alpha_over_window[%d][%d][%d] = %g\n", 0 , 0, index_alpha, alpha_over_window[0][0][index_alpha]);
    /*for (int index_r = 0; index_r < pgb2->r_size; index_r++) {
  ...
  for(int index_type_first = 0; index_type_first < pgb2->type_size; index_type_first++){
    ...
    for(int index_type_second = 0; index_type_second < index_type_first+1; index_type_second++){
      for (int bin1 = 0; bin1 < 1/*ppt->selection_num*/; bin1++) {
        for (int bin2 = 0; bin2 < 1/*ppt->selection_num*/; bin2++) {
          printf("[bin1][bin2] = [%d][%d]\n", bin1,bin2);
...
/*
            ...
            printf("alpha[0][0][0] = %g\n", alpha2[0][0][0] );
            printf("alpha[0][0][last] = %g\n", alpha2[0][0][pgb2->alpha_size-1]);
