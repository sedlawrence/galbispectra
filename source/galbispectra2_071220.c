     ...
     )
{
  ...
  pgb2->r_size = 1; //previously on 1501 1515
  ...
  pgb2->k_size_bessel = 5000; // prev 2500 4999 /*tau_size_bessel*/   /*k_size * boost*/ // formerly on 2500
...
/* Allocate and fill array for the trapezoidal weights for line of sight integration */
  ...
  int * selection_mean_index;
  class_alloc(selection_mean_index,
  ...
  for (bin = 0; bin < ppt->selection_num; bin++) {
    ...
    for (index_tau = 0; index_tau < pgb2->tau_size_selection; index_tau++) {
      ...
      selection_mean_index[bin] = index_selection_mean;
  }
        ...
        if(pgb2->index_type_quad_density_p != -1){
          ...
          for (int index_l = 0; index_l < ptr->l_size[ppt->index_md_scalars]; index_l++) {
            for (int index_tau = 0; index_tau < pgb2->tau_size_cls; index_tau++){
              ...
              //printf("pgb2->first_order_sources_integ[pgb2->index_type_quad_density_p][%d][%d][%d] = %g\n", index_l, index_tau, index_k_bessel, pgb2->first_order_sources_integ[pgb2->index_type_quad_density_p][index_l][index_tau][index_k_bessel] );
              //printf("prefactor_quad_density_p = %g\n", prefactor_quad_density_p );
              //printf("j_first_deriv = %g\n", j_first_deriv );
              //printf("pgb2->first_order_sources[pgb2->index_source_delta_cdm][index_tau][index_k_bessel] = %g\n", pgb2->first_order_sources[pgb2->index_source_delta_cdm][index_tau][index_k_bessel] );
              }
            }
          }
        }
            ...
            for (int index_tau = 0; index_tau < pgb2->tau_size_cls; index_tau++){
              ...
              for (int index_k_bessel = 0; index_k_bessel < pgb2->k_size_bessel; index_k_bessel++) {
               ...
               double prefactor_quad_v_pp = k
                ...
                //printf("pgb2->first_order_sources_integ[pgb2->index_type_quad_v_pp][%d][%d][%d] = %g\n", index_l, index_tau, index_k_bessel, pgb2->first_order_sources_integ[pgb2->index_type_quad_v_pp][index_l][index_tau][index_k_bessel] );
                //printf("prefactor_quad_v_pp = %g\n", prefactor_quad_v_pp);
                //printf("j_third_deriv = %g\n", j_third_deriv );
                //printf("pgb2->first_order_sources[pgb2->index_source_theta][index_tau][index_k_bessel] = %g\n", pgb2->first_order_sources[pgb2->index_source_theta][index_tau][index_k_bessel] );
              }
            }
          }
        }
  ...
  //  }
    ...
    for (int index_l = 0; index_l < ptr->l_size[ppt->index_md_scalars]; index_l++) {
      ...
      for (int bin1 = 0; bin1 < 1; bin1++) {
        int index_tau_first_min = selection_mean_index[bin1];
        for (int bin2 = 0; bin2 < 1; bin2++) {
          int index_tau_second_min = selection_mean_index[bin2];
                  ...
                  //  printf("DAC12 = %g \n", DAC12);
                    //printf("last_index_l_DBD13 = %d\n", last_index_l_DBD13);
                     ...
                     interpolate_Dl_for_l(l3,
                      ...
                      /*class_call_parallel (drc3jj (abs_M3,
                                                   l3,
                                                   -M3,
                                                   M3,
                                                   &min_D,
